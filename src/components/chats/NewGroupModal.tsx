// Modal for creating a new group. Title + member multi-select. Submitting
// calls Messaging.createGroup and navigates to the new conversation.
...
export function NewGroupModal({ onClose }: { onClose: () => void }) {
  ...
  const canSubmit =
    title.trim().length >= 1 && members.length >= 1 && !submitting;
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const r = await messagingClient.createGroup({
        title: title.trim(),
    ...
    <div className="fixed inset-0 z-30 flex items-start justify-center bg-bg/80 pt-20 px-4">
      ...
      >
        <div className="px-5 pt-5 pb-3 border-b border-line flex items-center justify-between">
          <Kicker className="mb-0">new group</Kicker>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-5 py-5 space-y-5">
          ...
          <div>
            <label className="kicker block mb-2">members</label>
            <UserMultiPicker value={members} onChange={setMembers} />
